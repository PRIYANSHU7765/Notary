...
const NotaryAssetsPage = () => {
  ...
  const sessionId = authUser?.sessionId || authUser?.currentSessionId || localStorage.getItem('notary.lastSessionId') || userId || `session-${Date.now()}`;
  ...
  const loadData = async () => {
    try {
      ...
      const [fetchedAssets, fetchedSignatures] = await Promise.all([
        fetchAssets('notary', { userId, sessionId: fallbackSessionId }),
        fetchSignatures('notary', { userId, sessionId: fallbackSessionId }),
      ]);
  ...
  return (
    <NotaryWorkspaceShell sidebarRole="notary" title="Assets" subtitle="Notary asset management">
                ...
                <div style={{ marginBottom: '8px' }}>
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      setUploadAssetType(isPdf ? 'pdf' : 'image');
...
export default NotaryAssetsPage;
